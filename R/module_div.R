...
module_server_div<-function (input,output,session,vals,df_colors,newcolhabs,df_symbol ){
  ...
  output$Hloghelp<-renderUI({
    ...
    column(12,style="background: white",
           ...
           p("Shannon Function 'H' on base 2. This term is the measure corresponding to the entropy concept defined by:",withMathJax(helpText("$$ H = \\sum_{n=1}^n (p_i*\\l2 p_i)$$")))
    )
  })
  output$Hlog10help<-renderUI({
    ...
    column(12,style="background: white",
           ...
           p("Shannon Function 'H' on base 10. This term is the measure corresponding to the entropy concept defined by:",withMathJax(helpText("$$ H = \\sum_{n=1}^n (p_i*\\l10 p_i)$$")))
    )
  })
