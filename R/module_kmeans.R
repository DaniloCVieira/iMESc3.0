...
output$oc_boot<-renderUI({
  ...
  observeEvent(input$oc_run,{
    ...
    gap_stat <- clusGap(getdata_kmeans(), FUN = kmeans, K.max = input$k.max,iter.max = input$km_itermax, nstart = input$km_nstart,algorithm = input$km_alg, B = input$oc_boot)
