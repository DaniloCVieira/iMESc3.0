...
#' @export
module_server_som2<-function (input,output,session,vals,df_colors,newcolhabs,df_symbol ){
  ...
  output$showgrid <- renderPlot({
    ...
    try({
      ...
      m <-
        supersom(
          as.matrix(matrix(sample(1:100,nrow(data)*ncol(data),replace=T),nrow(data),ncol(data))),
   ...
   # validate(need(length(vals$saved_data)>0,"No Datalist found"))
    ...
    div(id=ns("som_mode"),
      span(
        ...
        inline(radioButtons(ns("som_type"), NULL, choices=c("Unsupervised", "Supervised"), inline=T, selected='Supervised'))
      )
    )
  })
