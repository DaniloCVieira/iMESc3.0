...
#beep()
  ...
  observeEvent(input$run_preds,{
    ...
    #compre<-readRDS('compre.rds')
    #input<-readRDS('input.rds')
    #beep()
    
    
    
    #saveRDS(  vals$res_compre,"compre.rds")
    #beep()
  ...
  getpredictions<-reactive({
    #saveRDS(vals$listamodels2,"listamodels2.rds")
    #saveRDS(vals$res_compre,"res_compre.rds")
    #vals<-list()
    #vals$listamodels2<-readRDS("listamodels2.rds")
    #vals$res_compre2<-readRDS("res_compre.rds")
    #beep()
  ...
  combb_create_pred<-reactive({
    ...
    attr(datao,"factors")<-temp
    
    
    if(input$hand_save=="create") {
      vals$saved_data[[input$newdatalist]]<-datao
    } else{
      vals$saved_data[[input$over_datalist]]<-datao
    }
