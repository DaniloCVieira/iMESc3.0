...
module_server_downcenter <- function (input, output, session,vals ){
  ...
  getdown<-reactive({
    switch(vals$hand_down,
           ...
           "sgboost - training errors (observations)"=   data.frame(vals$sgboost_down_errors_train),
           "sgboost - predictions"=data.frame(vals$sgboosttab_pred),
           "sgboost - variable importance"=data.frame(vals$sgboost_varImp),
           "sgboost - table"=data.frame(vals$sgboost_tableClass),
           "sgboost_cm"=data.frame(vals$sgboost_cm),
