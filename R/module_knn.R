...
module_server_knn <- function (input, output, session,vals,df_colors,newcolhabs ){
  ...
  get_parts_knn<-reactive({
    ...
    partition<- attr(vals$saved_data[[input$data_knnY]],"factors")[, input$knn_test_partition]
  ...
  getdata_knnY<-reactive({
    ...
    if(input$knn_type=="Classification"){
      labels <- attr(data,"factors")
      labels<-labels[,input$knn_sup]
      labels
    } else{
      data<-data[,input$knn_sup]
      data
    }
  ...
  observe({
    req(input$data_knnX)
    req(isFALSE(insert_knn_resutls$df))
    req(length(vals$knn_unsaved)>0|length(attr(vals$saved_data[[input$data_knnX]],"knn"))>0)
    insertTab('knn_tab',target='knn_tab1',
              tabPanel(strong("2. Results"),value='knn_tab2',
                       uiOutput(ns("results_knn"))),

    )
    insertTab('knn_tab',target='knn_tab2',
              tabPanel(strong("3. Predict"),value='knn_tab3',
                       uiOutput(ns("predict_knn")))
    )
    insert_knn_resutls$df<-T
  })
  observeEvent(input$predknn_new,{
    vals$predknn_new<-input$predknn_new
  })
