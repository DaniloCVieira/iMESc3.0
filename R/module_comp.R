...
module_ui_comp <- function(id){
  ...
  column(12,uiOutput(ns('COMB_COMP')))
...
module_server_comp <- function (input, output, session,vals,df_colors,newcolhabs,df_symbol){
  ...
  observeEvent(input$palette,{vals$cur_comp_palette<-input$palette})

  observeEvent(input$point_size,{vals$point_size<-input$point_size})
  observeEvent(input$axis_size,{vals$axis_size<-input$axis_size})
  observeEvent(input$label_size,{vals$label_size<-input$label_size})
  observeEvent(input$lwd_size,{vals$lwd_size<-input$lwd_size})
  observeEvent(input$shape,{vals$cur_comp_shape<-input$shape})
  ...
  observeEvent(input$comp_downplot,{
  ...
  observeEvent(input$comp_results,{
  ...
  output$page2<-renderUI({
    ...
    div(
      tabsetPanel(id=ns("comp_results"),selected=vals$comp_results,
                  tabPanel("1.1. Summary",value="tab1",
                           div(style="overflow-x: scroll",uiOutput(ns('summary')))),
                  tabPanel("1.2. bwplot",value="tab2",
                           uiOutput(ns('plot1'))),
                  tabPanel("1.3. densityplot",value="tab3",
                           uiOutput(ns('plot2'))),
                  tabPanel("1.4. dotplot",value="tab4",
                           uiOutput(ns('plot3'))),
                  tabPanel("1.5. parallelplot",value="tab5",
                           uiOutput(ns('plot4'))),
                  tabPanel("1.6. splom",value="tab6",
                           uiOutput(ns('plot5'))),
                  tabPanel("1.7. xyplot",value="tab7",
                           uiOutput(ns('plot6'))),
                  tabPanel("1.8. P-values (pairwise comparisons)",
                           value="tab8",
  ...
  limodels<-lapply(attributes,function(x) attr(data,x))
  ...
  insert_npages <- 2
  observeEvent(input$comp_prev, navPage(-1))
  observeEvent(input$comp_next, navPage(1))
  ...
  output$plot2<-renderUI({
    ...
    renderPlot({
      ...
      vals$comp_plot<-lattice::densityplot(
        ...
        par.settings = list(superpose.line = list(col = getcolhabs(newcolhabs,input$palette,length(vals$modellist)))),
    ...
    div(
      renderPlot({
       ...
       # #trellis.par.set("par.ylab.text",mt)
        vals$comp_plot<- lattice::parallelplot(results,cex=input$point_size,pch=as.numeric(input$shape), lwd=input$lwd_size, auto.key =T, par.settings = list(superpose.line = list(col = getcolhabs(newcolhabs,input$palette,length(vals$modellist)))))
        vals$comp_plot
      })
    )
  })
