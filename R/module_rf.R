...
observeEvent(input$rf_tab,{
  ...
  trainrf_loop<-reactive({
    ...
    saveRDS(reactiveValuesToList(vals),"savepoint_loop.rds")
    ...
    output$RF_predictions<-renderUI({
      column(12,style="background: white", uiOutput(ns("predictions_rf")),
             ...
             tabsetPanel(id=ns("predrf_tab"),
                         ...
                         tabPanel(
                           strong("3.2. Forest prediction"), value="rftab3_2",
                           sidebarLayout(
                             sidebarPanel(
                               uiOutput(ns("qclass_side")),
                               uiOutput(ns("r_forests"))
                             ),
                             mainPanel(
                         ...
                         tabPanel(
                           strong("3.3. Performace"), value="rftab3_3",
                           div(style="background: white",
  ...
  observeEvent(input$dc_cmTest_byclass,{
  ...
  observeEvent(input$dc_cmTest_overall,{
  ...
  observeEvent(input$dowcenter_cmTest_rf,{
  ...
  observeEvent(input$inter_frame_tab,{
 ...
 # savereac()
  ...
  observeEvent(input$rf_useinter,{
    vals$rf_useinter<-input$rf_useinter
  })
  observeEvent(input$rfbi_grid,{
...
observeEvent(input$pd_size_plot,{
  vals$pd_size_plot<-input$pd_size_plot
})
observeEvent(input$pd_palette,{
...
#isolate(saveRDS(reactiveValuesToList(input),"input.rds"))
 ...
 # vals<-readRDS("vals.rds")
  ...
  observeEvent(input$inter_size_plot,{
    vals$inter_size_plot<-input$inter_size_plot
  })
  observeEvent(input$inter_palette,{
    vals$inter_palette<-input$inter_palette
  })
  observeEvent(input$inter_labang,{
  ...
  observeEvent(input$size_mmd,{vals$size_mmd<-input$size_mmd})
  observeEvent(input$labrfsize,{vals$labrfsize<-input$labrfsize})
  observeEvent(input$ylab_prf,{vals$ylab_prf<-input$ylab_prf})
  observeEvent(input$xlab_prf,{vals$xlab_prf<-input$xlab_prf})
  ...
  output$side_rf_mindeph<-renderUI({
    ...
    if(is.null(vals$size_mmd)){vals$size_mmd<-4}
    if(is.null(vals$labrfsize)){vals$labrfsize<-10}

    if(is.null(vals$ylab_prf)){vals$ylab_prf<-"Variable"}
    if(is.null(vals$xlab_prf)){vals$xlab_prf<-"Number of Trees"}
    ...
    fluidRow(class="map_control_style",style="color: #05668D",
             ...
             div(class="well3",
                 ...
                 div(
                   span("+ X label:",
                        inline(
                          textInput(ns("xlab_prf"),NULL, value=vals$xlab_prf, width="200px")
                        )
                   )
                 ),
                 div(
                   span("+ Y label:",
                        inline(
                          textInput(ns("ylab_prf"),NULL, value=vals$ylab_prf, width="200px")
                        )
                   )
                 ),
                 ...
                 div(
                   span("+ Size:",
                        inline(
                          numericInput(ns("labrfsize"),NULL, value=vals$labrfsize, width="75px")
                        )
                   )
                 ),
                 div(
                   span("+ Text-in size:",
                        inline(
                          numericInput(ns("size_mmd"),NULL, value=vals$size_mmd, width="75px")
                        )
                   )
                 ),
...
output$saved_rfs<-renderUI({
  ...
  output$rf_tab_errors0_pred<-DT::renderDataTable({
    ...
    rownames(table)<-input$rf_models
    vals$rf_preddatalist<-table
    req(length(table)>0)
  ...
  observeEvent(input$down_rf_preddatalist,{
  ...
  observeEvent(input$obs_cm_pred,{
...
sidebarLayout(
  ...
  get_parts_rf<-reactive({
    ...
    if(input$rf_test_partition!='None'){
      ...
      parts=list(train=train,test=test)} else{
...
output$summ_trees<-renderPlot({
  ...
  observeEvent(input$downp_summ_trees,{
  ...
  observeEvent(input$q_class_toptrees,{
  ...
  observeEvent(input$rf_create_predictions,{
  ...
  observeEvent(input$down_rf_tab3_1,{
...
req(input$interframe_palette)
  ...
  name_saverf<-reactive({
    name0<-paste0("RF (",attr(vals$RF_results,"Y"),'~',input$data_rfX,")")
    bag<-1
    # name0<-paste0("rf")
