...
app_server <- function(input, output, session) {
  ...
  observeEvent(input$tabcomb,{
    vals$tabcomb<-input$tabcomb
  })
  output$menu_comp_out<-renderUI({
    column(12,tabsetPanel(id='tabcomb', selected=vals$tabcomb,
                          tabPanel(strong("1. Compare models"),value="tabcomb1",
                                   uiOutput('menu_comp_out1')
                          ),
                          tabPanel(strong("2. Combine models for predictions"),value="tabcomb2",
                                   uiOutput('menu_comp_out2')
                          )
  ...
  output$menu_comp_out2<-renderUI({
    module_ui_comp2("module_comp2")
  })
  observeEvent(input$tabcomb,{
    req(input$tabcomb=='tabcomb2')
    callModule(module_server_comp2, "module_comp2",  vals=vals, df_colors=vals$colors_img,newcolhabs=newcolhabs)
  })


  observeEvent(input$tabcomb,{
    req(input$tabcomb=='tabcomb1')
    callModule(module_server_comp, "module_comp",  vals=vals, df_colors=vals$colors_img,newcolhabs=newcolhabs,df_symbol=df_symbol)
  })

  output$menu_comp_out1<-renderUI({
    module_ui_comp("module_comp")
  })
  ...
  observeEvent(input$pt_palette,{vals$pt_palette<-input$pt_palette})
  observeEvent(input$showcoords,{vals$showcoords<-input$showcoords})
  observeEvent(input$showfactors,{vals$showfactors<-input$showfactors})
  observeEvent(input$nmax,{vals$nmax<-input$nmax})
  observeEvent(input$nmax_idp,{vals$nmax_idp<-input$nmax_idp})
  observeEvent(input$nmin_idp,{vals$nmin_idp<-input$nmin_idp})
  observeEvent(input$omax,{vals$omax<-input$omax})
  observeEvent(input$maxdist,{vals$maxdist<-input$maxdist})
  observeEvent(input$idp,{vals$idp<-input$idp})
  observeEvent(input$scalesize_color,{vals$scalesize_color<-input$scalesize_color})
  observeEvent(input$scalesize_size,{vals$scalesize_size<-input$scalesize_size})
  observeEvent(input$pt_factor,{vals$pt_factor<-input$pt_factor})
  observeEvent(input$pt_coords,{vals$pt_coords<-input$pt_coords})
  observeEvent(input$pt_points_min,{vals$pt_points_min<-input$pt_points_min})
  observeEvent(input$pt_points,{vals$pt_points<-input$pt_points})
  observeEvent(input$pt_legend,{vals$pt_legend<-input$pt_legend})
  observeEvent(input$showguides,{vals$showguides<-input$showguides})
  observeEvent(input$col_guides,{vals$col_guides<-input$col_guides})
  observeEvent(input$lty_guides,{vals$lty_guides<-input$lty_guides})
  observeEvent(input$lwd_guides,{vals$lwd_guides<-input$lwd_guides})
  observeEvent(input$base_col,vals$base_col<-input$base_col)
  observeEvent(input$base_lighten,vals$base_lighten<-input$base_lighten)
  observeEvent(input$base_col_border,vals$base_col_border<-input$base_col_border)
  observeEvent(input$pt_symbol,{vals$pt_symbol<-input$pt_symbol})
  observeEvent(input$scale_map,{vals$scale_map<-input$scale_map})
  observeEvent(input$colored_map,{vals$colored_map<-input$colored_map})
  observeEvent(input$layer_col,vals$layer_col<-input$layer_col)
  observeEvent(input$layer_lighten,vals$layer_lighten<-input$layer_lighten)
  observeEvent(input$layer_col_border,vals$layer_col_border<-input$layer_col_border)
  observeEvent(input$col_coords,{vals$col_coords<-input$col_coords})
  observeEvent(input$col_factor,{vals$col_factor<-input$col_factor})
  ...
  output$sr_3dcontrol<-renderUI({
    absolutePanel(draggable=T,style = "z-index: 9999999999;" ,## z-index modification,
           fluidRow(class="well2",
                    "+ 3D control",
                    column(12,
                           div(tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"a expansion factor applied to the z coordinates. Often used with 0 < expand < 1 to shrink the plotting box in the z direction", placement = "right"),
                               '+ exp:',
                               inline(numericInput("stack_exp",NULL, 1, width="75px", step=.1))
                           ),
                           div(tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"azimuthal direction", placement = "right"),
                               '+ theta:',
                               inline(numericInput("stack_theta",NULL, 0, width="75px",step=10))
                           ),
                           div(tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"colatitude direction", placement = "right"),
                               '+ phi:',
                               inline(numericInput("stack_phi",NULL, 40, width="75px", step=5))
                           ),
                           div(tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"rhe distance of the eyepoint from the centre of the plotting box", placement = "right"),
                               '+ Eye point:',
                               inline(numericInput("stack_eye",NULL, 1.73, width="75px"))
                           ),
                           div(
                             tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"a value which can be used to vary the strength of the perspective transformation. Values of d greater than 1 will lessen the perspective effect and values less and 1 will exaggerate it", placement = "right"),
                             '+ persp strength:',
                             inline(numericInput("stack_d",NULL, 1, width="50px"))
                           ),
                           div(tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"Transparency of the layers", placement = "right"),
                               '+ Transparency:',
  ...
  observeEvent(input$sr_zlab,vals$sr_zlab<-input$sr_zlab)
  observeEvent(input$sr_ticktype,vals$sr_ticktype<-input$sr_ticktype)

  output$sr_label_options<-renderUI({
    if(is.null(vals$sr_zlab)){vals$sr_zlab<-"Layer"}
    div("+ Label options",
        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"'simple' draws just an arrow parallel to the axis to indicate direction of increase; 'detailed' draws normal ticks as per 2D plots", placement = "bottom"),
               span("+ Ticktype:",
                    pickerInput("sr_ticktype",NULL,choices=c("detailed", "simple"),inline=T, width="75px"), selected=vals$sr_ticktype)
        ),
        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"X axis label", placement = "bottom"),
               span("+ xlab:",
                    textInput("sr_xlab",NULL,"Longitude", width="100px"))
        ),
        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"Y axis label", placement = "bottom"),
               span("+ ylab:",
                    textInput("sr_ylab",NULL,"Latitude", width="100px"))
        ),
        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"Z axis label", placement = "bottom"),
               span("+ zlab:",
                    textInput("sr_zlab",NULL,vals$sr_zlab, width="100px"))
        ),
        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"the (approximate) number of tick marks to draw on the axes", placement = "bottom"),
               span("+ sr_nticks:",
                    numericInput("sr_nticks",NULL, value=5, width="100px", step=1))
        ),


        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"Show axes", placement = "bottom"),
               span("+ Show axes:",
                    checkboxInput("box_all",NULL,T, width="100px"))
        ),
        column(12,tipify(icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle",style="color: gray"),"Show borders", placement = "bottom"),
               span("+ Show border:",
                    checkboxInput("box_border",NULL,F, width="100px"))
        )
    )
  })
    ...
    column(12,
           fluidRow(
             ...
             span(
               ...
               div(class="palette",
                   span("+ Color:",
                        inline(tags$div(class = "ident-picker", pickerInput(inputId = "srlayer_col",label = NULL,choices =   vals$colors_img$val[getsolid_col()],choicesOpt = list(content =   vals$colors_img$img[getsolid_col()]),options=list(container="body"),inline=F, width="80px", selected="gray")))
                   ),
                   span("+ Transp:",
  ...
  observeEvent(input$smd_3dcontrol,{
  ...
  observeEvent(input$ss3d_exp,vals$ss3d_exp<-input$ss3d_exp)
  observeEvent(input$ss3d_theta,vals$ss3d_theta<-input$ss3d_theta)
  observeEvent(input$ss3d_phi,vals$ss3d_phi<-input$ss3d_phi)
  observeEvent(input$ss3d_eye,vals$ss3d_eye<-input$ss3d_eye)
  observeEvent(input$ss3d_d,vals$ss3d_d<-input$ss3d_d)
  ...
  observeEvent(input$sslayer_lighten,vals$sslayer_lighten<-input$sslayer_lighten)
  observeEvent(input$sslayer_col,vals$sslayer_col<-input$sslayer_col)
  output$ss3d<-renderUI({

    req(isTRUE(input$stack_scatter_3d))
    validate(need(length(vals$ss_map)>1,"Add at least two variables to generate the plot"))
    #ss_map<-readRDS('ss_map.rds')
...
tipify(span('Breakpoints'), "The break points computed", placement = "right")
  ...
  output$map_1e_limits<-renderUI({
    ...
    column(12,
           div(class="well2",
               ...
               div(
                 div(inline(div("+ Limits:",style="color: #05668D;width:55px")),inline(div("Long",style="width:150px")),inline(div("Lat",style="width:150px"))),
                 div(
                   splitLayout("+ min:",cellWidths = c("50px",'150px',"150px"),
                               numericInput("long_xmin",NULL, value=limits[1,1], width="150px", step=0.01),
                               numericInput("lat_xmin",NULL, limits[1,2], width="150px", step=0.01))),
                 div(
                   splitLayout("+ max:",cellWidths = c("50px",'150px',"150px"),
                               numericInput("long_xmax",NULL, value=limits[2,1], width="150px", step=0.01),
                               numericInput("lat_xmax",NULL, limits[2,2], width="150px", step=0.01)))
  ...
  observe({
   ...
   output$map_out<-renderUI({
    ...
    if(input$choices_map=="Numeric-Attribute"){
      if(input$cmap=='discrete'){disc_data_map()}
      if(input$cmap=='interpolation'){interp_data_map()}
      if(input$cmap=='raster'){raster_map()}
    }
  ...
  observeEvent(vals$cur_tab,{
    req(menu_steps$df[position()]!=input$tabs)
    req(length(vals$saved_data)>0)
if(is.null(menu_steps$anext)&is.null(menu_steps$prev)){

    menu_steps$df<-c(menu_steps$df,vals$cur_tab)
    menu_steps$num<-c(menu_steps$num,length(menu_steps$df))
    position(length(menu_steps$df))
    } else{
      menu_steps$anext<-NULL
      menu_steps$prev<-NULL
    }
  ...
  gettitle<-reactive({
    div(style="color: #05668D", id="menu-title",
        ...
        switch(input$tabs,
               ...
               'menu_som2'={
                 h4(strong("Supervised Self-Organizing Maps"),
                    actionLink('somhelp',icon(verify_fa = FALSE,name=NULL,class="fas fa-info-circle")))
               },
               'menu_knn'={
  ...
  observeEvent(input$gethelp_rf,{
    output$help_menurf<-renderText({
      getHelp(help(randomForest, "randomForest"))
    })
  })
  observeEvent(input$gethelp_train,{
    output$help_menurf<-renderText({
      getHelp(help(train, "caret"))
    })
  })
  observeEvent(input$gethelp_trainControl,{
    output$help_menurf<-renderText({
  ...
  output$datalist_cogs<-renderUI({
    ...
    div(id="dcogs",
        ...
        div(id="dlinks_all",class='dlinks_all',
            ...
            div(class="dlinks",
                actionLink("dcogs8","Edit Model names")
            ),
            div(class="dlinks",
  ...
  observeEvent(input$confirm_newmodelnames,{
  ...
  observeEvent(input$editmodel_at,{
  ...
  output$editmodel_model<-DT::renderDT({
    DT::datatable(vals$oldmodelsnames, editable = TRUE, options=list(info = FALSE,lengthMenu = list(c(-1), c("All")), autoWidth=T,dom = 't'))
  })

  observeEvent(input$editmodel_model_cell_edit, {
  ...
  observeEvent(input$editmodel_targ,{
    vals$cur_data<-input$editmodel_targ
  })
  ...
  observeEvent(input$editmodel_pick,{
  ...
  output$bank_input0<-renderUI({
    ...
    div(style="margin-top: 5px;",
        div(class="get_started",
            ...
            div(icon(verify_fa = FALSE,name=NULL,class="fas fa-hand-point-up", style="font-size: 30px; color: SeaGreen")),div(em("Get started by creating a Datalist"))
  ...
  output$tools_bank<-renderUI({
    ...
    attributes<-list(
      ...
      if(check_model(data,"rf")){ div(id="007",span(icon(verify_fa = FALSE,name=NULL,class="fas fa-tree"),icon(verify_fa = FALSE,name=NULL,class="fas fa-tree", style = "margin-left: -8px;"),icon(verify_fa = FALSE,name=NULL,class="fas fa-tree", style = "margin-left: -8px;")))},
      ...
      if(check_model(data,"knn")) { div(id="010", img(src=knn_icon2,height='20',width='20'))},
      if(check_model(data,"sgboost")) { div(id="011", img(src=sgboost_icon2,height='20',width='20'))},
    ...
    div(
      ...
      bsPopover("011",NULL,"SGboost-Attribute"),
  ...
  observeEvent(vals$updeldata,{
    ...
    pic<-vals$updeldata
    vals$updeldata<-NULL
    data<-attr(delcol_values$tab,'data')[,pic]
  ...
  observeEvent(list(input$editdata,input$editattr),{
  ...
  observeEvent(input$tabcolnames_cell_edit, {
  ...
  observeEvent(input$rename_datalist,{vals$rename_datalist<-input$rename_datalist})
  ...
  observeEvent(input$prev_merge, navpage_merge(-1))
  observeEvent(input$next_merge, navpage_merge(1))
  ...
  observeEvent(input$transp_confirm,{
  ...
  observeEvent(input$replace_factors,{
    replace_fac_modal()
  })
  ...
  observeEvent(input$newfactorfile,{
    ...
    labels <-data.frame(fread(input$newfactorfile$datapath, stringsAsFactors = T,na.strings=c("","NA")))
  ...
  observeEvent(input$comments_edit,{
  ...
  observeEvent(input$comments_remove,{
  ...
  observeEvent(input$qnote_open,{
    showModal( modal_comment())
  })
  observeEvent(input$confirm_note,{
    vals$note<-gsub("\n",'<br/>',input$note_new)
  ...
  #global knn
  {
    getglobal_knn<-reactive({
      ...
      models<-lapply(attr(vals$saved_data[[input$data_bank]],"knn"), function (x) x[[1]])
  ...
  #global sgboost
  {
    getglobal_sgboost<-reactive({
      ...
      models<-lapply(attr(vals$saved_data[[input$data_bank]],"sgboost"), function (x) x[[1]])
      res<-caret_global_stats(models)
      res
    })
    output$sgboost_remove<-renderUI({
      div(
        popify(bsButton("trash_sgboost",icon(verify_fa = FALSE,name=NULL,class="fas fa-edit"), style="button_active"),NULL,"Select and remove a sgboost")
      )
    })

    observeEvent(input$trash_sgboost ,{
      showModal(
        modalDialog(easyClose = T,
                    title=strong("Remove sgboost-Attribute"),
                    div(
                      div(
    ...
    observeEvent(input$delete_sgboost,{
    ...
    output$viewsgboost<-renderUI({
      ...
      div(
        h4(strong("sgboost-Attribute"),inline(uiOutput('sgboost_remove'))),
    ...
    observeEvent(input$down_sgboosttable_reg,{
      vals$hand_down<-"sgboost_stats_reg"
      module_ui_downcenter("downcenter")
      mod_downcenter <- callModule(module_server_downcenter, "downcenter",  vals=vals)
    })
    observeEvent(input$down_sgboosttable_class,{
  ...
  to_merge_datarow<-reactive({
    ...
    transfer_rownames<-function(datalist, dataframe){
      ...
      if(!length(dup_rows)>0){
        rownames(dataframe)<-do.call(c,li)
      }
      return(dataframe)
  ...
  output$insert_page1<-renderUI({
    div(
      h4("Create Datalist",
         popify(actionLink("uphelp0",icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle")),"Data bank structure",as.character(paste0(
  ...
  output$insert_reset_btns<-renderUI({
    div(class="topa6",
        div(
          div(class="topa7",
              if(length(input$filedata$datapath)>0){  bsButton("reset_insert_1",icon(verify_fa = FALSE,name=NULL,class="fas fa-undo"), style="button_change")}),
          div(class="topa7",
              if(length(input$labels$datapath)>0){bsButton("reset_insert_2",icon(verify_fa = FALSE,name=NULL,class="fas fa-undo"), style="button_change")}),
          div(class="topa7",
              if(length(input$coords$datapath)>0){bsButton("reset_insert_3",icon(verify_fa = FALSE,name=NULL,class="fas fa-undo"), style="button_change")}),
          div(class="topa7",
  ...
  observeEvent(load_qsave(),{
    ...
    #req("saved_data"%in%names(newvals))
    # newvals<-readRDS('savepointddd.rds')

    #vals<-readRDS("savepoint.rds")
    # newvals<-readRDS("savepoint_WILL.rds")
    #colsjoin<-c(names(vals$newcolhabs),names(newvals$newcolhabs))
    #colslist<-c(vals$newcolhabs,newvals$newcolhabs)
    #colsjoin[which(!duplicated(colsjoin))]
    #colslist[which(!duplicated(colsjoin))]
    # dfcolors<-do.call()

    #newvals$colors_img[colsjoin,]<-dfcolors[colsjoin,]

    # vals$newcolhabs
    #vals$newcolhabs[-names(newcolhabs_list)]<-get('matlab.like2')
    #vals$newcolhabs[["turbo"]]<-get('turbo')
  ...
  output$pct_rareabund<-renderUI({
    div(style="margin-left: 5px",
        tipify( icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle"),"Remove variables with an value less than x-percent of the total"),
  ...
  output$pct_rarefreq<-renderUI({
    div(style="margin-left: 5px",
        tipify( icon(verify_fa = FALSE,name=NULL,class="fas fa-question-circle"),"Remove variables occurring in less than  x-percent of the number of samples"),
...
?persp
  ...
  output$smw_data_WSS_out <- renderUI({
  ...
  output$smw_model_WSS_out <- renderUI({
    req(!is.null(vals$screeplot_results))
    renderUI({
      div(
  ...
  output$data_WSS_out <- renderUI({
    req(input$model_or_data=='data')
    div(inline(uiOutput("K_WSS_data")),
        inline(actionButton("getWSSdata", "run",width="50px", style = "animation: glowing3 1000ms infinite;")),
        div(uiOutput("smw_data_WSS_out"))
    )
  })

  output$model_WSS_out <- renderUI({
    req(input$model_or_data=='som codebook')
    div(inline(uiOutput("K_WSS_model")),
        inline(actionButton("getWSSmodel", "run",width="50px", style = "animation: glowing3 1000ms infinite;")),
        div(uiOutput('smw_model_WSS_out'))
    )
  ...
  observeEvent(input$getWSSmodel, {
    withProgress(message = "Running elbow analysis... this may take a while....",
                 min = 1,
                 max = 1,
                 {
  ...
  observeEvent(vals$screeplot_results,{
  ...
  output$K_WSS_data<-renderUI({
    ...
    div(tipify(strong("+ k"),"maximum number of clusters to be tested"),
        numericInput("K_WSS_data", NULL, value = Kdata(getdata_hc()), width="100px"))

  })
  output$K_WSS_model <- renderUI({
    div(tipify(strong("+ k"),"maximum number of clusters to be tested"),
        numericInput("K_WSS_model", NULL, value = myKmodel(getmodel_hc()), width="100px"))
  ...
  output$hc_tab2_out<-renderUI({
    ...
    div(
      ...
      div(uiOutput('plot_model_WSS')),
      div(uiOutput('plot_data_WSS'))
  ...
  observeEvent(input$down_results_screeplot,{
  ...
  observeEvent(input$mapcode_loop_go,{
    ...
    withProgress(message = "Please, wait!",
                 ...
                 max = K,
                 {
                   ...
                   #input$show_mapcode_errors
                 })


    vals$mapcode_loop_res<-list(res_mean=res_mean, K=K)
  ...
  observeEvent(input$mapcode_loop_go,{
    output$mapcode_loop<-renderUI({
      ...
      res_mean=unlist(lapply(lapply(res_mean,function(x) x[,input$show_mapcode_errors]),mean))
      K<-vals$mapcode_loop_res$K

      renderPlot({

        abline(v=1:K,col="gray", lty=3)

        plot(1:K,unlist(res_mean), xlab="N clusters", ylab=
  ...
  observeEvent(input$mapcode_title,{vals$mapcode_title<-input$mapcode_title})
  observeEvent(input$show_mapcode_errors,{vals$show_mapcode_errors<-input$show_mapcode_errors})
  ...
  observeEvent(input$mapcode_p_symbol,
  ...
  observeEvent(input$mapcode_p_bg_transp,{
  ...
  observeEvent(input$mapcode_p_border_grid,{
  ...
  output$saveHC<-renderUI({
    ...
    if(isTRUE(vals$baghc)) {div(class="border_alert",
                                bsButton("tools_savehc", icon(verify_fa = FALSE,name=NULL,class="fas fa-save"),style  = "button_active", type="action",value=FALSE)
    )} else{
      div(style="margin-bottom: 5px",
          em("Clusters saved in the Factor-Attributte")
      )
    }
  })
  ...
  output$hist_d1<-renderUI({
    renderPlot({
      req(is.data.frame(data_cogs$df))
      data<-as.vector(unlist(data_cogs$df))
      hist(data)
    })
  })
  ...
  output$hist_d2<-renderUI({
    renderPlot({
      req(is.data.frame(data_cogs$df))
      colSums<-colSums(data_cogs$df,na.rm = T)
      hist(colSums)
    })
  })
  output$hist_d3<-renderUI({
    renderPlot({
      req(is.data.frame(data_cogs$df))
      rowSums<-rowSums(data_cogs$df,na.rm = T)
      hist(rowSums)
    })
  })
  ...
  output$change_background<-renderUI({
    ...
    class  = if(sum(vals$bagdata==c(T,T))==2){"novo"} else{"change_alert"}
  ...
  output$lock_back<-renderUI({
    ...
    div(id="fade",class="needed",
        ...
        column(12,id="fade_panel",class="side_panel"))
  })
  ...
  output$tools_upload<-renderUI({
    ...
    choiceNames=list(
      ...
      div(id="tools_drop5",class="needed",div(cogs_title="Data partition",
                                              img(src=split_icon,height='14',width='14'))),
      div(id="tools_drop8",class="needed",div(cogs_title="Aggregate",
    ...
    if(length(vals$saved_data)>0){
      div(
        ...
        inline(div(id="exit_cogs",    uiOutput("preprocess_exit"))),inline(div(
      ...
      div(div(
        ...
        inline(
          div(
            ...
            inline(
              tags$div(class="cogs_null",style="
                 width: 35px;
                 height: 35px;
                 position: absolute;
                 right: 5px;
                 ...
                 margin: 0px;
                 line-height: 30px",id="coog0_NULL",
  ...
  output$preprocess_exit<-renderUI({
    ...
    req(!is.na(input$radio_cogs))
    req(isTRUE(vals$last_cog))
    tipify(actionButton("exit_preprocess",icon(verify_fa = FALSE,name=NULL,class="fas fa-window-close")),
           'Close')
  })

  observeEvent(input$exit_preprocess,{
    runjs("Shiny.setInputValue('last_btn', 'fade');")
  })
  ...
  observeEvent(list(input$data_upload,input$reset_change, input$savechanges_cancel,input$showtree),{
    ...
    output$tree <- renderTree({
      ...
      #req(input$showtree %% 2)
      #input<-readRDS("savepoint.rds")
      #vals<-readRDS("vals.rds")
      #input$data_upload<-names(vals$saved_data)[3]
  ...
  observeEvent(input$tree,{
    ...
    vals$seltree<-get_selfactors(res=get_selected(input$tree),factors=vals$factor_tree)
    #beep(10)
  })
      ...
      div(id="tool8",
          ...
          splitLayout(
            ...
            div(
              ...
              div(
                bsButton("go_agg", img(src=agg_icon2,height='14',width='14'),style  = "button_active", type="action",value=FALSE, block=F),
                inline(uiOutput("save_agg_btn")),
  ...
  output$save_agg_btn<-renderUI({
    if(is.data.frame(vals$agg)){
      div(class='save_changes',bsButton("tools_saveagg", icon(verify_fa = FALSE,name=NULL,class="fas fa-file-signature"),style  = "button_active", type="action",value=FALSE, block=F),

          bsTooltip('tools_saveagg','Click to create a Datalist with the aggregation results'))

    } else{
      div(em(icon(verify_fa = FALSE,name=NULL,class="fas fa-hand-point-left"),"Click to aggregate"))
    }
  ...
  output$filterby_indobs<-renderUI({
    ...
    #data<-getdata_upload()
    div(style="overflow-x: scroll;height:250px;overflow-y: scroll",
        validate(need(nrow(vals$saved_data[[vals$cur_data]])<1000,"Only available for data with less than 5000 observations")),
  ...
  observeEvent(vals$cur_data,
               output$filterby_indvars<-renderUI({
                 ...
                 div(style="margin-top: 10px;",
                     ...
                     div(class="cogs_in",style='overflow-x: scroll;overflow-y: scroll;height:250px;',
                         div(
  ...
  output$imesc_panel<-renderUI({
    ...
    column(12,id="teste",class="needed",
  ...
  fn_download <- function()
  {
    ...
    switch (vals$hand_plot,
            ...
            "mantel plot"=replayPlot(vals$map_res),
  ...
  output$plotoutput <- renderImage({
    ...
    if(vals$hand_plot=="mantel plot"){replayPlot(vals$map_res)}
