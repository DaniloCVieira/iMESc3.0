...
module_server_sgboost <- function (input, output, session,vals,df_colors,newcolhabs ){
  ...
  observeEvent(input$predsgboost_newY_tree,{
  ...
  output$sgboost_pred_type<-renderUI({
    ...
    div(inline(radioButtons(ns("sgboostpred_which"),"New data (X):",choices=choices,inline=T, width="200px")), inline(uiOutput(ns("datalist_pred_sgboost"))))
  })

  observeEvent(input$predsgboost_new,{
    vals$predsgboost_new<-input$predsgboost_new
  })
  output$datalist_pred_sgboost<-renderUI({
    req(input$sgboostpred_which =='Datalist')
    div(
      pickerInput(ns("predsgboost_new"),"Datalist",names(vals$saved_data[getnewdatasgboost()]), width = '300px', selected=vals$predsgboost_new)
    )
  })
  ...
  output$confusion_sgboost_pred<-renderUI({
    ...
    column(12,
           renderPlot({
             res<-plotCM(conf/sum(conf)*100, input$sgboostpalette_pred,  newcolhabs=vals$newcolhabs)
  ...
  observeEvent(input$dowcenter_cmsgboost_pred,{
    vals$hand_down<-"sgboost_cm_test"
    module_ui_downcenter("downcenter")
    mod_downcenter <- callModule(module_server_downcenter,"downcenter", vals=vals)
  })
        ...
        fluidRow(class="map_control_style",style="color: #05668D",
                 div(
                   span("+",tipify(icon("fas fa-question-circle"),"Data containing observed values to be compared with predicted values"),
                        span(supname),
                        inline(pickerInput(ns("predsgboost_newY"),
                                           NULL,names(vals$saved_data[getobssgboost()]), width="200px"))
                   )
                 ),
                 div(
                 ...
                 div(tipify(actionLink(ns("downp_cmsgboost_pred"), span("+ Donwload",icon("fas fa-download"),icon("fas fa-image")),style  = "button_active"),NULL,"Download plot")),
                 div(tipify(actionLink(ns("dowcenter_cmsgboost_pred"),span("+ Donwload",icon("fas fa-download"),icon("fas fa-image")),style  = "button_active"),NULL,"Download plot"))

        )
      ),
      mainPanel(
        tags$style(
          paste(paste0("#",ns('sgboost_tab_errors0_test')),"td {padding: 3px;
                     text-align: left;
                     font-size:12px}")
        ),
        tags$style(
          paste0("#",ns('sgboost_tab_errors0_test')),"th {padding: 3px;
                     text-align: left;
                     font-size:12px}"
        ),
        div(
          ...
          fluidRow(
            ...
            div(
              tipify(
                ...
                "Create a datalist with the sgboost predictions", options=list(container="body")
              )
            ),
            div(
              tipify(
  ...
  output$sgboost_inputs <- renderUI({
    if(is.null(vals$cur_data_sgboostY)){vals$cur_data_sgboostY<-1}
    if(is.null(vals$cur_sgboost_type)){vals$cur_sgboost_type<-"Classification"}
  ...
  output$data_sgboostY_out<-renderUI({
    div(
      div("Datalist:"),
      div(pickerInput(ns("data_sgboostY"),NULL,choices =  names(vals$saved_data), width="150px", selected=vals$cur_data_sgboostY))
    )
  })
  output$sgboost_Y <- renderUI({
    ...
    if(is.null(vals$cur_sgboost_sup)){vals$cur_sgboost_sup<-1}
    ...
    div(well="class2",
        ...
        div(tipify(pickerInput(
          ...
          NULL,
          choices =choices ,
          width="150px", selected=vals$cur_sgboost_sup
        ),"The response vector"))
    )
  ...
  output$sgboost_partition<-renderUI({
    ...
    if(is.null(vals$cur_sgboost_test_partition)){vals$cur_sgboost_test_partition<-1}
    div(
      div(span("Partition:",tiphelp("choose a factor as reference for the partition"))),
      div(pickerInput(ns("sgboost_test_partition"),NULL, choices=c("None", colnames(attr(vals$saved_data[[input$data_sgboostY]],"factors"))), width="150px", selected=vals$cur_sgboost_test_partition))
    )
  })
  output$sgboost_test_ref<-renderUI({
    req(input$sgboost_test_partition!="None")
    req(input$data_sgboostY)
    if(is.null(vals$cur_testdata_sgboost)){vals$cur_testdata_sgboost<-1}
    ...
    div(
      ...
      pickerInput(ns("testdata_sgboost"),NULL, choices=choices, width="200px", selected=vals$cur_testdata_sgboost)
    )
  })
  ...
  output$sgboost_results_menu<-renderUI({
    ...
    div(pickerInput(ns("sgboost_models"),strong("sgboost results:", tiphelp("Stochastic Gradient Boosting Results. Click to select sgboost results saved in the Training Datalist (X).")), choices= names(attr(vals$saved_data[[input$data_sgboostX]],"sgboost")), width="200px", selected = vals$cur_sgboost_models)
    )
  })
  output$rmsgboost_button<-renderUI({
    req(input$sgboost_models!="new sgboost (unsaved)")
    div(style="margin-top: 20px",
        tipify(actionButton(ns("tools_rmsgboost"), icon("far fa-trash-alt")), "Remove the sgboost model from the training Datalist (X)")
    )
  ...
  output$sgboost_tab_2_1<-renderUI({
    ...
    div(
      ...
      div(style="height: 100px",
          popify(downloadButton(ns("down_sgboost_results"),label=span("Download sgboost model",img(src=sgboost_icon,height='20',width='20')),style = "button_active"),NULL,
                 HTML(paste0(
  ...
  output$down_sgboost_results <- {
    downloadHandler(
      filename = function() {
        paste0("sgboost","_", Sys.Date(),".rds")
      }, content = function(file) {
        saveRDS(vals$sgboost_results,file)
      })
  }
  observeEvent(input$sgboost_tab2,{
  ...
  output$results_sgboost<-renderUI({
    ...
    column(12,
           ...
           tabsetPanel(id=ns("sgboost_tab2"),selected=vals$sgboost_tab2,
                       tabPanel("1. Summary",value="sgboost_tab2_1",
                                uiOutput(ns("sgboost_tab_2_1"))
                       ),
                       tabPanel("2. Performace",value="sgboost_tab2_2",
                                uiOutput(ns("sgboost_tab_2_2"))
                       ),
                       tabPanel("3. Variable importance",
                                sidebarLayout(
                                  sidebarPanel(
                                    uiOutput(ns("sgboost_tab_2_3side"))
                                  ),
                                  mainPanel(  uiOutput(ns("sgboost_tab_2_3")))
                                )

                       ),
                       tabPanel("4. Confusion matrix",value="sgboost_tab2_4",
  ...
  output$sgboost_tab_2_3side<-renderUI({
    req(length(vals$sgboost_results))
    m<-vals$sgboost_results
    Y<-which(colnames(m$trainingData)==".outcome")
    nvars<-ncol(m$trainingData[,-Y])
    ...
    div(
      ...
      div("+ Number of variables:",
          numericInput(ns("varImp_n_sgboost"),NULL,value=val,step=1,max=nvars,width="80px")
      ),
      ...
      div(
        tipify(
          actionLink(
            ns('sgboost_varImp'),span("+ Download",icon("fas fa-download"),icon("fas fa-table")),style="button_active"
          ),
      ...
      div(
        tipify(
          actionLink(
            ns('sgboost_varImp_plot'),span("+ Download",icon("fas fa-download"),icon("fas fa-image")),style="button_active"
          ),
  ...
  output$sgboost_tab_2_3<-renderUI({
    req(vals$sgboost_results)
    req(input$varImp_n_sgboost)
    renderPlot({
      m<-vals$sgboost_results
      vals$sgboost_varImp<-caret::varImp(m)$importance
      vals$sgboost_varImp_plot<-plot_varimport(m,palette=input$varimp_col,vals$newcolhabs,input$varimp_pos, type=input$varimp_type, nvars=input$varImp_n_sgboost)
  ...
  observeEvent(input$dowcenter_cmsgboost,{
    vals$hand_down<-"sgboost_cm"
    module_ui_downcenter("downcenter")
    mod_downcenter <- callModule(module_server_downcenter, "downcenter",  vals=vals)
  })
    ...
    obs<-if(input$sgboostpred_which=="Partition"){
      attr(m,"sup_test")} else{
        if(m$modelType=="Classification"){
          attr(vals$saved_data[[input$predsgboost_newY]],"factors")[,attr(m,"supervisor")]} else{
  ...
  test_sgboost<-reactive({
    req(input$sgboostpred_which)
    pred_tab<-if(input$sgboostpred_which=="Partition"){attr(vals$sgboost_results,"test")} else {
  ...
  get_parts_sgboost<-reactive({
    ...
    partition<- attr(vals$saved_data[[input$data_sgboostY]],"factors")[, input$sgboost_test_partition]
    test<-which(partition==input$testdata_sgboost)
    train<-which(partition!=input$testdata_sgboost)
  ...
  getdata_sgboostY<-reactive({
    ...
    if(input$sgboost_type=="Classification"){
      labels <- attr(data,"factors")
      labels<-labels[,input$sgboost_sup]
      labels
    } else{
      data<-data[,input$sgboost_sup]
      data
    }
  ...
  observe({
    req(input$data_sgboostX)
    req(isFALSE(insert_sgboost_resutls$df))
    req(length(vals$sgboost_unsaved)>0|length(attr(vals$saved_data[[input$data_sgboostX]],"sgboost"))>0)
    insertTab('sgboost_tab',target='sgboost_tab1',
              tabPanel(strong("2. Results"),value='sgboost_tab2',
                       uiOutput(ns("results_sgboost"))),

    )
    insertTab('sgboost_tab',target='sgboost_tab2',
              tabPanel(strong("3. Predict"),value='sgboost_tab3',
                       uiOutput(ns("predict_sgboost")))
    )
    insert_sgboost_resutls$df<-T
  })
  observeEvent(input$predsgboost_new,{
    vals$predsgboost_new<-input$predsgboost_new
  })
  observeEvent(input$sgboostpred_which,{
    vals$sgboostpred_which<-input$sgboostpred_which
  })
  observeEvent(input$sgboost_down_errors_train,{
    vals$hand_down<-"sgboost - training errors (observations)"
    module_ui_downcenter("downcenter")
    mod_downcenter <- callModule(module_server_downcenter, "downcenter",  vals=vals)
  })
  observeEvent(input$sgboost_varImp,{
    vals$hand_down<-"sgboost - variable importance"
    module_ui_downcenter("downcenter")
    mod_downcenter <- callModule(module_server_downcenter, "downcenter",  vals=vals)
  })
  observeEvent(input$down_sgboost_tab3_1,{
    vals$hand_down<-"sgboost - predictions"
    module_ui_downcenter("downcenter")
    mod_downcenter <- callModule(module_server_downcenter, "downcenter",  vals=vals)
  })
  observeEvent(input$data_sgboostY,{
    vals$cur_data_sgboostY<-input$data_sgboostY
  })
  observeEvent(input$sgboost_type,{
    vals$cur_sgboost_type<-input$sgboost_type
  })

  observeEvent(input$sgboost_method,{
    vals$cur_sgboost_method<-input$sgboost_method
  })
  observeEvent(input$sgboost_sup,{
    vals$cur_sgboost_sup<-input$sgboost_sup
  })
  observeEvent(input$data_sgboostX,{
    vals$cur_data<-input$data_sgboostX
  })
  observeEvent(input$sgboost_test_partition,{
    vals$cur_sgboost_test_partition<-input$sgboost_test_partition
  })
  observeEvent(input$testdata_sgboost,{
    vals$cur_testdata_sgboost<-input$testdata_sgboost
  })
  observeEvent(input$sgboost_models,{
    vals$cur_sgboost_models<-input$sgboost_models
  })
  observeEvent(input$trainsgboost,{
    ...
    beep(10)
    attr(vals$saved_data[[input$data_sgboostX]],"sgboost")[['new sgboost (unsaved)']]<-vals$sgboost_unsaved
    vals$cur_sgboost_models<-"new sgboost (unsaved)"
    vals$bag_sgboost<-T
    #saveRDS(vals$sgboost_unsaved,"sgboost.rds")
  })
  observeEvent(input$downp_cmsgboost,{
    vals$hand_plot<-"sgboost - Confusion Matrix"
    module_ui_figs("downfigs")
    mod_downcenter <- callModule(module_server_figs, "downfigs",  vals=vals)})
  observeEvent(input$downp_cmsgboost_pred,{
    vals$hand_plot<-"sgboost - Confusion Matrix (predictions)"
    module_ui_figs("downfigs")
    mod_downcenter <- callModule(module_server_figs, "downfigs",  vals=vals)})
  observeEvent(input$sgboost_varImp_plot,{
    vals$hand_plot<-"sgboost - Variable Importance plot"
    module_ui_figs("downfigs")
    mod_downcenter <- callModule(module_server_figs, "downfigs",  vals=vals)
  })
  observeEvent(input$downp_sgboost_dens,{
  ...
  observe({
    ...
    if(input$sgboost_models=="new sgboost (unsaved)"){
  ...
  observeEvent(input$tools_rmsgboost,{
    attr(vals$saved_data[[input$data_sgboostX]],"sgboost")[input$sgboost_models]<-NULL
  })
  observeEvent(input$tools_savesgboost,{
    if(input$tools_savesgboost %% 2){
      vals$hand_save<-"Save sgboost model in"
      vals$hand_save2<-column(12,fluidRow(em(input$data_sgboostX, style="color:gray"),strong("::"), em("sgboost-Attribute", style="color:gray"),strong("::")
      ))
      vals$hand_save3<-NULL
  ...
  observeEvent(input$sgboost_create_errors_train,{
    vals$hand_save<-"Create Datalist: sgboost training errors -obs"
    vals$hand_save2<-NULL
    vals$hand_save3<-NULL
    showModal(module_sgboost())
  })
  observeEvent(input$sgboost_create_predictions,{
    vals$hand_save<- "Create Datalist: sgboost predictions"
  ...
  name_resample_create<-reactive({
    ...
    name0<-paste("sgboost",var,"qclass", sep="_")
  ...
  get_newname<-reactive({
    ...
    newname$df<-switch(
      vals$hand_save,
      ##RF
      "Save sgboost model in"= { name_save_sgboost()},
      "Create Datalist: sgboost training errors -obs"={name_sgboost_train_errors()},
  ...
  name_sgboost_train_errors<-reactive({
    bag<-1
    name0<-paste0("sgboost training errors")
    name1<-paste(name0,bag)
    if(name1%in%names(vals$saved_data)){
      repeat{bag<-bag+1
      name1<-paste(name0,bag)
  ...
  name_sgboost_pred<-reactive({
    bag<-1
    name0<-paste0("sgboost predictions")
    name1<-paste(name0,bag)
    if(name1%in%names(vals$saved_data)){
      repeat{bag<-bag+1
      name1<-paste(name0,bag)
  ...
  output$data_over<-renderUI({
    ...
    if(vals$hand_save=='Save sgboost model in'){choices<-names(attr(vals$saved_data[[input$data_sgboostX]],'sgboost'))}
  ...
  observeEvent( input$data_confirm,{
    ...
    switch(
      vals$hand_save,
      "Save sgboost model in"= {savesgboost()},
      "Create Datalist: sgboost training errors -obs"=sgboost_create_training_errors(),
      "Create Datalist: sgboost predictions"={sgboost_create_pred()},
